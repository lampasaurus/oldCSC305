...
{    
    ...
    vec4 ambient = fColor * 0.2;                                                                    //Sets the ambient colouring
    vec4 diffuse = clamp((lightCol + 3 * fColor)/4 * max(dot(N, L), 0.0), 0.0, 1.0) * 1.0;          //Diffuse reflection = diffuseColor * angle
    vec4 specular = clamp((lightCol + fColor)/2 * pow(max(dot(R,E),0.0),shin) , 0.0, 1.0) * 0.4;    //Specular reflection = specularColor * reflection ^ shinyness
    if(dot(N, L) <= 0)
