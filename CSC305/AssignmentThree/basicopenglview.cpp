...
void BasicOpenGLView::loadMaterial(std::string m_Material)
{
    ...
    while(std::getline(buffer, line))
    {
        ...
        {
            //Looks up the image, and converts it to a GLFormat image
            std::string texturePath = filePath + line.substr(texture.size() + 1);
            QImage loadImage;
            loadImage.load(texturePath.c_str());
            QImage image = convertToGLFormat(loadImage);


            //Generates and binds texture, does some parameter stuff I dont understand, and generates a mipmap of the texture
        ...
        else if(line.compare(0, bump.size(), bump) == 0)
        {
             ...
             * @todo assignment 3
             * use the
             * to load an image from disc, and transfer its data to
